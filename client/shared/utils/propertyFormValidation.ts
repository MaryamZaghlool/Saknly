  ...
  export function validatePropertyForm(formData: any) {
    ...
    if (!formData.contactInfo.name) {
      ...
      if (!nameRegex.test(formData.contactInfo.name)) {
        newErrors['contactInfo.name'] = 'الاسم يجب أن يحتوي على أحرف عربية فقط';
        isValid = false;
    ...
    if (!formData.contactInfo.phone) {
      ...
      if (!phoneRegex.test(formData.contactInfo.phone)) {
        newErrors['contactInfo.phone'] = 'رقم الهاتف يجب أن يحتوي على أرقام فقط';
        isValid = false;
      } else if (formData.contactInfo.phone.length !== 11) {
        newErrors['contactInfo.phone'] = 'رقم الهاتف يجب أن يكون 11 رقم';
        isValid = false;
      }
    }
