...
const AgencyDetailsPage: React.FC = () => {
  ...
  const handleTestimonialSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTestimonialSuccess("تم إرسال رأيك بنجاح! سيتم مراجعته من الإدارة.");
      setTestimonialName("");
      setTestimonialText("");
    } catch (err: any) {
    ...
    return (
      <Box sx={{ minHeight: '100vh', backgroundColor: '#f8f9fa', py: 4 }}>
        <Container maxWidth="md">
          <Skeleton variant="rectangular" height={220} sx={{ borderRadius: 4, mb: 3 }} />
  ...
  if (error || !agency) {
    return (
      <Box sx={{ minHeight: '100vh', backgroundColor: '#f8f9fa', py: 4 }}>
        <Container maxWidth="md">
          <Alert severity="error" sx={{ mb: 2 }}>{error || "الوكالة غير متاحة"}</Alert>
        </Container>
      </Box>
    );
  }
  ...
  return (
    <>
      ...
      <Box sx={{ minHeight: '100vh', backgroundColor: '#f8f9fa', py: 4 }}>
        <Container maxWidth="md">
          {/* Agency Header */}
          <Paper sx={{ p: 4, mb: 4, bgcolor: 'rgba(255,255,255,0.95)', borderRadius: 4, border: '1px solid #eee', display: 'flex', alignItems: 'center', gap: 4 }}>
            <Avatar src={agency.logo?.url} alt={agency.name} sx={{ width: 100, height: 100, border: '2px solid #667eea' }} />
          ...
          <Paper sx={{ p: 3, mb: 4, bgcolor: '#f0f0f0', borderRadius: 4, border: '1px solid #CCC' }}>
            <Typography variant="h6" fontWeight="bold" gutterBottom color="#764ba2">
              العقارات المميزة لدى الوكالة
            </Typography>
  ...
  <Box mb={3}>
    ...
    <Collapse in={showForm}>
      ...
      >
        <Typography fontWeight="bold" mb={1}>أضف رأيك عن الوكالة</Typography>
        <TextField
          label="اسمك"
        ...
        />
        <Typography variant="caption" color={testimonialText.length === 200 ? 'error' : 'text.secondary'} sx={{ alignSelf: 'flex-end' }}>
        ...
        >
          {testimonialLoading ? <CircularProgress size={22} /> : "إرسال الرأي"}
        </Button>
        {testimonialSuccess && (
          <Alert 
            severity="success" 
